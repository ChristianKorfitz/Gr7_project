...
\begin{figure}[H]
	...
	\caption{Scalogram from the corrected data of region 15 in the uncuffed recording of subject 1, where a dampening of the high spike and drift magnitudes induced by the jumps has been achieved.}
	\label{fig:scalogram_corr}
\end{figure} 
However, this method still has weak points. The in figures \ref{fig:raw15} - \ref{fig:corr15} shown region of interest is located in the center of the thermal image. Regions which are located in the outer area of the thermal image show a few jumps after the correction which are bigger than before (\ref{fig:corr23}). These sporadic extended jumps are due to the fact that the thermal image is more unstable in the outer areas than in the center, thus the pixel drift is increasing with increasing distance of the center. \fxnote{maybe mention that the regression is linear and will not fit all the data thereby}
\begin{figure}[H]
