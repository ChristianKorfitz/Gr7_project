...
Additionally there is also a drift occurring within each interval between two jumps, which hampers the correct data analysis. 
...
%\end{figure} 
However, this method still has weak points. In figures \ref{fig:raw15} - \ref{fig:corr15} ROIs is located in the center of the thermal image. ROI which are located in the outer area of the thermal image show a few jumps after the correction, illustrated on \figref{fig:corr23}. These extended jumps are due to the fact that the thermal image is more unstable in the outer areas than in the center, thus the pixel drift is increasing with increasing distance to the center of the thermal image\cite{olbrycht2015}. This means that in the corrected data of ROIs located in the outer areas of the thermal image artifacts still occur. 
\begin{figure}[H]
